...
def experiment(train_data,
               val_data,
               test_data,
               extract_contextual_embedding_generator = None,
    ...
    for _ in range(num_reps):
        ...
        model.fit(train_dataset, batch_size=batch_size, epochs=epochs, validation_data=val_dataset, verbose="auto" if verbose else 0, callbacks=callbacks)
        ...
        if progressive:
            yield np.average(f1s), np.std(f1s)
...
def objective(trial, train_data, val_data, test_data):
    ...
    for step, (f1, _) in enumerate(experiment(train_data,
                                val_data,
                                test_data,
                                extract_contextual_embedding_generator=extract_contextual_embedding_generator, 
...
class MyPruner(BasePruner):
    def __init__(self, warmup_steps, warmup_trials, percentile):
    ...
    def prune(self, study: optuna.study.Study, trial: optuna.trial.FrozenTrial) -> bool:
        ...
        if step:
            ...
            other_scores = [t.value for t in completed_trials]
            # Get the 25th percentile of the scores
            min_score = np.percentile(other_scores, self.percentile)
            return this_score < min_score
...
def main():
    ...
    print('Training size:', training_length)
    print('Validation dataset size:', validation_length)
    print('Test dataset size:', test_length)
    ...
    study = optuna.create_study(
        ...
        direction="maximize",
        load_if_exists=True,
        pruner=MyPruner(3, 20, 25)
    )
    part_objective = partial(objective, train_data=train_data, val_data=val_data, test_data=test_data)
    study.optimize(part_objective, n_trials=trials)
